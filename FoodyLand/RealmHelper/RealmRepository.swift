...
class RealmRepository {
    ...
    func createItem<T: Object>(item: T) -> Result<Void, RealmError> {
        do {
            try realm.write {
                realm.add(item)
    ...
    func fetchItem<T: Object>(type: T.Type) -> Results<T> {
        let items = realm.objects(type.self) // 타입 자체가 필요하므로
        
        return items
    } // 카테고리를 가져올 수도 있고 유저의 메모들을 가져올 수도 있기 때문에
    
    func updateItem(marketId: String, marketItem: UserDiary) -> Result<Void, RealmError> { // viewModel에서 값 비교후 다른게 있다면
...
//                oneItem.
    ...
    func deleteItem(marketId: String) -> Result<Void, RealmError> {
    ...
    func deleteCategory(text: String) -> Result<Void, RealmError> {
